...
private:
  ...
  ////PROGRAM SETTINGS HOOKS. TO BE SET DEPENDING ON THE FRAMERWORK/PLATFORM/GUI
  /// SPECIFIC CODE (IE: QT)

  static std::function<bool(const std::string &key, std::string &val)>
      str_from_settings;
  static std::function<bool(const std::string &key, int &val)>
      int_from_settings;
  static std::function<bool(const std::string &key, bool &val)>
      bool_from_settings;

  static std::function<bool(const std::string &key, const std::string &val)>
      str_to_settings;
...
public:
  ...
  */
  ...
  static void setNetworkLoop(uWS::Loop *l) { network_loop = l; }
  ...
  static void updatePrinterStatus() {
    if (selected_printer != nullptr) {
      device_status temp = (selected_printer == nullptr)
                               ? DISCONNECTED
  ...
  static bool emitEvent(std::string name);
  static void registerEventHandler(std::string name,
